...
GRAINS_REV=$(curl "http://admin:password@127.0.0.1:5984/grains/_design/grains" | python3 -c 'import sys, json; print(json.load(sys.stdin)["_rev"])')
...
HOPS_REV=$(curl "http://admin:password@127.0.0.1:5984/hops/_design/hops" | python3 -c 'import sys, json; print(json.load(sys.stdin)["_rev"])')
...
MALTS_REV=$(curl "http://admin:password@127.0.0.1:5984/malts/_design/malts" | python3 -c 'import sys, json; print(json.load(sys.stdin)["_rev"])')
...
YEASTS_REV=$(curl "http://admin:password@127.0.0.1:5984/yeasts/_design/yeasts" | python3 -c 'import sys, json; print(json.load(sys.stdin)["_rev"])')
...
RECIPES_REV=$(curl "http://admin:password@127.0.0.1:5984/recipes/_design/recipes" | python3 -c 'import sys, json; print(json.load(sys.stdin)["_rev"])')
